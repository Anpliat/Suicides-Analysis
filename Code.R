...
# final_data$region<-factor(countrycode(final_data[, "Country"], origin = 'country.name', destination = 'region'))
# In "Middle East & North Africa" region there is only Israel, which belongs to Middle East, so we changed it.
# levels(final_data$region)[4] <- 'Middle East (Israel)'

data_rel$continent<-factor(countrycode(data_rel[, "Country"], origin = 'country.name', destination = 'continent'))
# data_rel$region<-factor(countrycode(data_rel[, "Country"], origin = 'country.name', destination = 'region'))
# levels(data_rel$region)[4] <- 'Middle East (Israel)'

df_data_cleaned$continent<-factor(countrycode(df_data_cleaned[, "Country"], origin = 'country.name', destination = 'continent'))
# df_data_cleaned$region<-factor(countrycode(df_data_cleaned[, "Country"], origin = 'country.name', destination = 'region'))
# levels(df_data_cleaned$region)[4] <- 'Middle East (Israel)'

data_totals$continent<-factor(countrycode(data_totals[, "Country"], origin = 'country.name', destination = 'continent'))
# data_totals$region<-factor(countrycode(data_totals[, "Country"], origin = 'country.name', destination = 'region'))
# levels(data_totals$region)[4] <- 'Middle East (Israel)'

data_totals_alc$continent<-factor(countrycode(data_totals_alc[, "Country"], origin = 'country.name', destination = 'continent'))
# data_totals_alc$region<-factor(countrycode(data_totals_alc[, "Country"], origin = 'country.name', destination = 'region'))
# levels(data_totals_alc$region)[4] <- 'Middle East (Israel)'

df_totals_cleaned$continent<-factor(countrycode(df_totals_cleaned[, "Country"], origin = 'country.name', destination = 'continent'))
# df_totals_cleaned$region<-factor(countrycode(df_totals_cleaned[, "Country"], origin = 'country.name', destination = 'region'))
# levels(df_totals_cleaned$region)[4] <- 'Middle East (Israel)'


# reg11<-unique(final_data$Country[final_data$region=='East Asia & Pacific'])
# reg12<-unique(final_data$Country[final_data$region=='Europe & Central Asia'])
# reg13<-unique(final_data$Country[final_data$region=='Latin America & Caribbean'])
# reg14<-unique(final_data$Country[final_data$region=='Middle East & North Africa'])
# reg15<-unique(final_data$Country[final_data$region=='North America'])

# unique(final_data$Country[final_data$continent=='Americas'])
# unique(final_data$Country[final_data$continent=='Asia'])
# unique(final_data$Country[final_data$continent=='Europe'])
# unique(final_data$Country[final_data$continent=='Oceania'])
...
# teamp54[teamp54$Country=='Greece',]
# round(100*teamp54[teamp54$Country=='Greece',6],1)
# round(100*teamp54[teamp54$Country=='Netherlands',6],1)
# round(100*teamp54[teamp54$Country=='Poland',6],1)
...
# library(viridis)  # for: scale_color_viridis function
# install.packages("hrbrthemes")
# library(hrbrthemes)  # for: theme_ipsum function
